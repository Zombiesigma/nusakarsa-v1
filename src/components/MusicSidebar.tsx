...
import type { Music as InternalMusic, Book, MusicTrack } from '@/lib/types';
...
export function MusicSidebar({ bookId }: MusicSidebarProps) {
  ...
  const handleTogglePreview = async (track: MusicTrack) => {
    ...
    setIsPreviewLoading(trackId);
    try {
      let streamUrl: string | null = null;
      if (track.source === 'internal' && track.url) {
          streamUrl = track.url;
  ...
  const handleAddToPlaylist = async (track: MusicTrack) => {
    ...
    try {
      ...
      // For YouTube tracks, we don't store the URL as it's generated on-demand.
